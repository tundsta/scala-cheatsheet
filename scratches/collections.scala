...
// (in all three folds the type of the start value must be the same as the return value).

//The first constraint is that the start value must be a supertype of the object you're folding. In our first example we
// were folding on a type List[Int] and had a start type of Int. Int is a supertype of List[Int].
...
val invalidFold = (1 to 10).toList.map(Foo.apply).fold(0)( _ + _)//error type mismatch
